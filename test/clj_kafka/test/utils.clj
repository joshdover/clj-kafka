...
(defn create-config
  ...
  (let [base-config {"broker.id"                   "0"
    ...
    (KafkaConfig. (assoc base-config "port" (str kafka-port)))))

;; enable.zookeeper doesn't seem to be used- check it actually has an effect
(defn create-broker
  [{:keys [kafka-port zookeeper-port]}]
      (KafkaServer. (create-config kafka-port zookeeper-port) system-time (Option/empty)))
...
(defmacro with-test-broker
  ...
  `(do (FileUtils/deleteDirectory (file (tmp-dir)))
       (let [zk# (create-zookeeper ~config)
         ...
         (try
           (.startup kafka#)
           (let [zk-utils# (admin/zk-utils (str "127.0.0.1:" (:zookeeper-port ~config))
                                             {:session-timeout-ms 500
                                              :connection-timeout-ms 500})]
             (admin/create-topic zk-utils# topic#)
